...
struct GiftVoucherView: View {
    @State private var amount = ""
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                Text("Gutschein kaufen").font(EntfaltaTheme.segoe(28, bold: true))
                Text("Das perfekte Geschenk für Freunde und Familie. Einlösbar für alle Produkte im Shop.")

                TextField("Betrag in EUR (z.B. 25)", text: $amount)
                    .textFieldStyle(.roundedBorder)
                ...
                Text("Gutschein vorhanden?").font(EntfaltaTheme.segoe(20, bold: true))
                Text("Du kannst deinen Code direkt im Warenkorb einlösen.")
                    .font(.caption)
...
struct OrderRow: View {
    ...
    var canReturn: Bool {
        let deliveredAt = order.createdAtMs // Assuming for now, normally order.deliveredAtMs
        let now = Date().timeIntervalSince1970 * 1000
        let fourteenDays = 14.0 * 24 * 60 * 60 * 1000
        return (now - deliveredAt) < fourteenDays && order.status != "closed"
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            Button { withAnimation { expanded.toggle() } } label: {
                VStack(alignment: .leading, spacing: 10) {
                    HStack {
                        Text(order.orderNumber).font(.headline).bold()
                        Spacer()
                    ...
                    HStack {
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            Text(date(order.createdAtMs)).font(.caption).foregroundStyle(.secondary)
                        }
                        Spacer()
                        statusBadge
                    }
            ...
            if expanded {
                ...
                if let tracking = order.trackingNumber, !tracking.isEmpty {
                    VStack(alignment: .leading, spacing: 10) {
                        HStack {
                            ...
                            Text("Sendung: \(tracking)")
                            Spacer()
                            if let url = order.trackingUrl {
                                Link("Tracking-Seite", destination: URL(string: url)!)
                                    .font(.caption)
                            }
                        }
                        .font(.caption.bold())
                        ...
                        if let trUrl = order.trackingUrl, (trUrl.contains("sendcloud.sc") || trUrl.contains("tracking.sc")) {
                            WebView(url: URL(string: trUrl)!)
                                .frame(height: 400)
                                .clipShape(RoundedRectangle(cornerRadius: 15))
                ...
                if canReturn {
                    ...
                    .alert("Rücksendung beantragen", isPresented: $showReturnAlert) {
                        Button("Abbrechen", role: .cancel) { }
                        Button("Beantragen") {
                            Task {
                                try? await FirebaseRest.shared.set(collection: "orders", id: order.id, values: ["returnRequest": ["status": "requested", "requestedAtMs": Date().timeIntervalSince1970 * 1000]], token: "")
                            }
                        }
                    } message: {
                        Text("Möchtest du eine Rücksendung beantragen?\n\nHinweis: Die Kosten für die Rücksendung werden vom Erstattungsbetrag abgezogen.\nDie normale Lieferzeit beträgt 2-3 Werktage, es kann jedoch zu Abweichungen kommen.")
                    }
                }
            }
        }
    ...
    private var statusBadge: some View {
        Text(order.status.uppercased())
            ...
            .padding(.vertical, 4)
            .background(EntfaltaTheme.leaf.opacity(0.2), in: Capsule())
            .foregroundStyle(EntfaltaTheme.leaf)
    }
}
